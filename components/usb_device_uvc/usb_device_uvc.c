...
// Heartbeat for external liveness monitoring: increments whenever the
// TinyUSB task completes an event-processing pass. If this stops advancing
// while tusb is initialized, the USB stack is wedged.
volatile uint32_t g_tud_task_heartbeat = 0;

static void tusb_device_task(void *arg)
{
    while (1) {
        tud_task_ext(100, false); // returns after 100ms idle so the heartbeat ticks
        g_tud_task_heartbeat++;
    }
}
